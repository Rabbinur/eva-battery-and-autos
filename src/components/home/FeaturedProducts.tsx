        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {t.featured.categories.map((cat, idx) => {
            const path = idx === 0 
              ? `/${lang}/rickshaws` 
              : idx === 1 
              ? `/${lang}/water-batteries` 
              : `/${lang}/spare-parts`;
            
            return (
              <Link href={path} key={idx} className="block group">
                <motion.article
                  initial={{ opacity: 0, scale: 0.95 }}
                  whileInView={{ opacity: 1, scale: 1 }}
                  viewport={{ once: true }}
                  transition={{ duration: 0.5, delay: idx * 0.1 }}
                  className="bg-white border border-slate-200 rounded-3xl p-6 flex flex-col justify-between hover:border-amber-500/30 transition group shadow-[0_15px_40px_rgba(0,0,0,0.03)] hover:shadow-[0_20px_45px_rgba(0,0,0,0.07)] h-full cursor-pointer"
                >
                  <div>
                  ...
                  <div className="w-full py-3 bg-slate-50 group-hover:bg-amber-500 group-hover:text-slate-950 font-bold rounded-xl text-xs sm:text-sm transition-all duration-300 flex items-center justify-center space-x-2 border border-slate-200 group-hover:border-transparent">
